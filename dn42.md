...
<script>
    ...
    const COPIES = [-3, -2, -1, 0, 1, 2, 3];
    const GHOST_COPIES = [-3, -2, -1, 1, 2, 3];
    ...
    const nodeDefs = [
        ...
        { id: 'fra', lat: loc_fra[0], lng: loc_fra[1], popup: "<b>DN42:</b> fra1.sherpherd.dn42<br><b>Clearnet:</b> fra1.sherpherd.net<br><b>Location:</b> Frankfurt, Germany<br><b>Bandwidth:</b> 10Gbps<br><br><b>MPLS Enabled</b><br>" },
        ...
        { id: 'cgk', lat: loc_cgk[0], lng: loc_cgk[1], icon: orangeIcon, popup: "Planned Site" },
    ...
    // ── Render nodes (main + ghost copies for infinite scroll) ─
    nodeDefs.forEach(function(node) {
        var opts = node.icon ? { icon: node.icon } : {};
        // Main marker at canonical position
        L.marker([node.lat, node.lng], opts).addTo(map)
            .bindPopup(node.popup).setZIndexOffset(1000);
        // Ghost copies in adjacent world copies (non-interactive, behind)
        GHOST_COPIES.forEach(function(offset) {
            L.marker([node.lat, node.lng + offset * 360],
                { icon: node.icon || new L.Icon.Default(), interactive: false, zIndexOffset: -100 }
    ...
    var linkDefs = [
        ...
        { from: 'can', to: 'sjc', style: {opacity: 0.8}, popup: "<b>Link Name: </b>CAN1 == SJC1<br><b>Estimated RTT Latency:</b> 160ms" },
        ...
        { from: 'can', to: 'fra', style: {opacity: 0.8}, popup: "<b>Link Name: </b>CAN1 == FRA1<br><b>Estimated RTT Latency:</b> 250ms" },
    ...
    // ── Render links (multi-world copies for infinite scroll) ──
    linkDefs.forEach(function(link) {
        ...
        COPIES.forEach(function(offset) {
            var shiftedPts = shiftPoints(pts, offset);
            var style = {};
            for (var k in link.style) { style[k] = link.style[k]; }
            style.noClip = true;
            L.polyline(shiftedPts, style).addTo(map).bindPopup(link.popup);
        });
    });
</script>
